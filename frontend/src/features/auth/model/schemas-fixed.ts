...
export const passwordRuleMessage =
  '비밀번호는 8자 이상 64자 이하이고, 영문과 숫자를 각각 1자 이상 포함해야 하며 공백은 사용할 수 없습니다.';

export const loginSchemaFixed = z.object({
  email: z.string().trim().email('email 형식을 확인해 주세요.'),
...
export const profileSchemaFixed = z
  .object({
    email: z.string().trim().email('email 형식을 확인해 주세요.').optional().or(z.literal('')),
    nickname: z.string().trim().min(1, '닉네임을 입력해 주세요.').max(30, '닉네임은 30자 이하여야 합니다.').optional(),
